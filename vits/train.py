...
def main(model: str, config: dict):
    ...
    optim_d = torch.optim.AdamW(
        ...
        eps=config['train']['eps'])
    try:
        g_checkpoint_path = f'./assets/{MODEL_NAME}/ckpt/G_.pth'
        ...
        d_checkpoint_path = f'./assets/{MODEL_NAME}/ckpt/D_.pth'
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(train_loader):
        ...
        if global_step % 1000 == 0:
            ...
            torch.save({'model': state_dict,
                ...
                'learning_rate': config['train']['learning_rate']}, f'./assets/{MODEL_NAME}/ckpt/G_{global_step}.pth')
            if hasattr(net_d, 'module'): state_dict = net_d.module.state_dict()
            else: state_dict = net_d.state_dict()
            torch.save({'model': state_dict,
                'iteration': epoch,
                'optimizer': optim_d.state_dict(),
                'learning_rate': config['train']['learning_rate']}, f'./assets/{MODEL_NAME}/ckpt/D_{global_step}.pth')
...
def get_logger(modelname: str):
    ...
    file_handler = logging.FileHandler(f'./assets/{modelname}/train.log')
...
if __name__ == "__main__":
    ...
    os.mkdir(f'./assets/{args.model}/ckpt')
    with open(f'./assets/{args.model}/config.json', 'r') as f:
        config = json.load(f)
